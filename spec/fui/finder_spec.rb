  ...
  context "included from a .m file" do
    ...
    describe "#headers" do
      it "finds all headers" do
        ...
        expect(finder.headers.map { |h| h.filename }.sort).to eq(["unused_class.h", "used_class.h"])
      end
    end
    describe "#references" do
      it "maps references" do
        ...
        expect(Hash[finder.references.map { |k, v| [ k.filename,  v.count ]}]).to eq({
          "unused_class.h" => 0,
          "used_class.h" => 1
        })
      end
    ...
    describe "#unsed_references" do
      it "finds unused references" do
        finder = Fui::Finder.new(@fixtures_dir)
        expect(Hash[finder.unused_references.map { |k, v| [ k.filename,  v.count ]}]).to eq({
          "unused_class.h" => 0
        })
      end
    end
